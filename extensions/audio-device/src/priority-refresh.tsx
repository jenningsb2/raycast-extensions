...
export default async function PriorityRefresh() {
  ...
  try {
    ...
    if (outputDevices.length === 0 || inputDevices.length === 0) {
      console.log(`[PriorityRefresh] No devices available - output: ${outputDevices.length}, input: ${inputDevices.length}`);
    ...
    console.log(`[PriorityRefresh] Top priority devices - output: ${topOutputDevice?.name || 'none'}, input: ${topInputDevice?.name || 'none'}`);
    ...
    if (shouldSwitchOutput && topOutputDevice) {
      console.log(`[PriorityRefresh] Switching output device from '${currentOutput.name}' to '${topOutputDevice.name}'`);
      try {
