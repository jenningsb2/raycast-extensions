...
export default async function PriorityMonitor() {
  ...
  try {
    ...
    console.log(`[PriorityMonitor] STEP 2: Output: ${currentOutput?.name || 'null'}, Input: ${currentInput?.name || 'null'}`);
    ...
    console.log(`[PriorityMonitor] STEP 4: Found ${outputDevices.length} output devices, ${inputDevices.length} input devices`);
    console.log(`[PriorityMonitor] STEP 4: Priority lists - output: ${outputPriorityList.length} items, input: ${inputPriorityList.length} items`);

    if (outputDevices.length === 0 || inputDevices.length === 0) {
      console.log(`[PriorityMonitor] STEP 4: No devices available - output: ${outputDevices.length}, input: ${inputDevices.length}`);
    ...
    console.log(`[PriorityMonitor] STEP 6: Is background: ${isBackground}, auto-switch enabled: ${preferences.enableAutoSwitch}`);

    if (isBackground && preferences.enableAutoSwitch) {
      // Check output device
      if (topOutputDevice && currentOutput.uid !== topOutputDevice.uid) {
        console.log(`[PriorityMonitor] STEP 6: Switching output device from '${currentOutput.name}' to '${topOutputDevice.name}'`);
      ...
      if (topInputDevice && currentInput.uid !== topInputDevice.uid) {
        console.log(`[PriorityMonitor] STEP 6: Switching input device from '${currentInput.name}' to '${topInputDevice.name}'`);
